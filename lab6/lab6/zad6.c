...
int mergeReeipts(receiptPosition newReceiptNode, receiptPosition currentReceiptInList);
...
int main() {
	...
	char dateFrom[MAX_LENGHT] = { '\0' }, dateTo[MAX_LENGHT] = { '\0' }, articleName[MAX_LENGHT] = { '\0' };

	readAndCreateReceiptList(&receiptHead);

	printf("\nZa saznati koliko je sveukupno novca potroseno i na koliku kolicinu odredenog artikla u nekom vremenskom razdoblju upisite ime tog artikla i trazeno vremensko razdoblje u formatu 'ime YYYY-MM-DD YYYY-MM-DD' : ");
...
receiptPosition createAndFillReceiptNode(char* receiptName) {
	FILE* filename = NULL;
	char receiptDate[MAX_LENGHT] = { 0 }, buffer[MAX_LINE] = { 0 }, articleName[MAX_LENGHT];
...
articlePosition createAndFillArticleNode(char* articleName, int articleQuantity, int articlePrice) {
	...
	newArticleNode->quanity = articleQuantity;
...
int articleSortedInput(articlePosition articleHead, articlePosition newArticleNode) {
	...
	if (current->next != NULL && strcmp(newArticleNode->name, current->next->name) == 0) {
		current->next->quanity += newArticleNode->quanity;
...
int receiptSortedInput(receiptPosition receiptHead, receiptPosition newReceiptNode) {
	...
	if (current->next != NULL && compareDates(newReceiptNode->receiptDate, current->next->receiptDate) == 0) {
		mergeReeipts(newReceiptNode, current->next);
		free(newReceiptNode);
	}
...
int mergeReeipts(receiptPosition newReceiptNode, receiptPosition currentReceiptInList) {
	...
	while (current->next != NULL) {
		temp = createAndFillArticleNode(current->next->name, current->next->quanity, current->next->price);
	...
	if (startReceipt == endReceipt) {
		...
		while (receiptCurrent != endReceipt->next) {
			...
			if (articleCurrent->next != NULL && strcmp(articleCurrent->next->name, wantedArticleName) == 0) {
				totalQuantity += articleCurrent->next->quanity;
